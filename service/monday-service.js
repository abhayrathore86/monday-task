...
exports.uploadFile = async(payload, res) => {
    if (!payload.event) {
    ...
    if (event.type === 'update_column_value' && event.columnId === 'status') {
        let itemId = payload.event.pulseId;
        let appendText = [];
        appendText.push(' ' + payload.event.columnTitle + ' - ' + payload.event.value.label.text);
...
function getUserDetails(personId) {
    ...
    return fetch(MONDAY_API_URL, setRequest('post', query))
        ...
        .then(userDetails => userDetails.data.users[0].account.name)
        .catch(err => console.log(err));
}
        ...
        switch (col.id) {
            case 'person':
                if (col.value && col.value.personsAndTeams) {
                    let person = await getUserDetails(col.value.personsAndTeams[0].id)
